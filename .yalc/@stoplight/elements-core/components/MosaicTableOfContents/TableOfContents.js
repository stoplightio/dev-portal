...
exports.TableOfContents = React.memo(({ tree, activeId, Link, maxDepthOpenByDefault }) => {
    React.useEffect(() => {
        ...
        React.createElement(mosaic_1.Box, { my: 3 },
            React.createElement(LinkContext.Provider, { value: Link },
                React.createElement(ActiveIdContext.Provider, { value: activeId }, tree.map((item, key) => {
                    ...
                    return React.createElement(GroupItem, { key: key, item: item, depth: 0, maxDepthOpenByDefault: maxDepthOpenByDefault });
                }))))));
});
...
const GroupItem = React.memo(({ item, depth, maxDepthOpenByDefault }) => {
    if (utils_1.isExternalLink(item)) {
        ...
        return React.createElement(Group, { depth: depth, item: item, maxDepthOpenByDefault: maxDepthOpenByDefault });
    }
...
const Group = React.memo(({ depth, item, maxDepthOpenByDefault }) => {
    const activeId = React.useContext(ActiveIdContext);
    const [isOpen, setIsOpen] = React.useState(() => {
        return utils_1.isGroupOpenByDefault(depth, item, activeId, maxDepthOpenByDefault);
    });
