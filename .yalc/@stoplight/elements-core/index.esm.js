...
import { isArray as isArray$1, Box, Flex, Icon, Button, Popover, Panel, CopyButton, Menu, twCss, CX, Text, Input, Select, FieldButton, Image, Link, useThemeIsDark, HStack, VStack, InvertTheme, Tooltip, Badge, LinkHeading, Tabs, TabList, Tab, TabPanels, TabPanel, Heading as Heading$1, useClipboard, useMosaicContext, Provider as Provider$1 } from '@stoplight/mosaic';
...
const ParameterGridCSS = twCss({
    '@apply': CX('grid', 'gap-1.5', 'items-center', 'pb-4', 'max-h-40', 'overflow-y-auto'),
...
const APIKeyAuth = ({ scheme, onChange, value }) => {
    return (React.createElement(Panel.Content, { className: ParameterGridCSS },
...
const FormDataBody = ({ specification, values, onChangeValues }) => {
    ...
    return (React.createElement(Panel, { defaultIsOpen: true },
        ...
        React.createElement(Panel.Content, { className: ParameterGridCSS }, mapSchemaPropertiesToParameters(parameters, required).map(parameter => {
...
const RequestBody = ({ examples, requestBody, onChange }) => {
    return (React.createElement(Panel, { defaultIsOpen: true },
        ...
        React.createElement(Panel.Content, { pb: 4, maxH: 48, overflowY: "auto" },
...
const OperationParameters = ({ parameters, values, onChangeValue, validate, }) => {
    return (React.createElement(Panel, { defaultIsOpen: true },
        ...
        React.createElement(Panel.Content, { className: ParameterGridCSS }, parameters.map(parameter => (React.createElement(ParameterEditor, { key: parameter.name, parameter: parameter, value: values[parameter.name], onChange: (value) => onChangeValue(parameter.name, String(value)), validate: validate }))))));
};
    ...
    return (React.createElement(Panel, { defaultIsOpen: true },
        ...
        React.createElement(Panel.Content, null,
            React.createElement("div", null,
                ...
                response.bodyText && responseType && ['json', 'xml', 'text'].includes(responseType) ? (React.createElement(CodeViewer, { language: "json", value: responseType && bodyFormat === 'preview'
...
const TryIt = ({ httpOperation, mockUrl, onRequestChange, requestBodyIndex, embeddedInMd = false, tryItCredentialsPolicy, corsProxy, className, }) => {
    ...
    const tryItPanelContents = (React.createElement(React.Fragment, null,
        ((_d = httpOperation.security) === null || _d === void 0 ? void 0 : _d.length) ? (React.createElement(TryItAuth, { onChange: setOperationAuthValue, operationSecurityScheme: (_e = httpOperation.security) !== null && _e !== void 0 ? _e : [], value: operationAuthValue })) : null,
        ...
        React.createElement(Box, { className: "SendButtonHolder", p: 4, bg: { default: 'canvas-tint', inverted: 'transparent' } },
    ...
    let tryItPanelElem;
    if (embeddedInMd) {
        tryItPanelElem = (React.createElement(Panel, { hasChildPanels: true, isCollapsible: false, p: 0, className: "TryItPanel", rounded: true },
            React.createElement(Panel.Titlebar, { bg: "canvas-200" },
        ...
        tryItPanelElem = (React.createElement(Box, { className: "TryItPanel", rounded: "lg" }, tryItPanelContents));
    }
    return (React.createElement(Box, { rounded: "lg", overflowY: "hidden", className: className },
        tryItPanelElem,
...
const VersionBadge = ({ value }) => (React__default.createElement(Badge, { appearance: "solid", size: "sm" }, enhanceVersionString(value)));
...
const SidebarLayout = React.forwardRef(({ sidebar, children, maxContentWidth = MAX_CONTENT_WIDTH, sidebarWidth = SIDEBAR_WIDTH }, ref) => {
    ...
    return (React.createElement(Flex, { ref: ref, className: "sl-elements-api", pin: true, h: "full" },
        React.createElement(Flex, { direction: "col", bg: { default: 'canvas-50', dark: 'transparent' }, borderR: true, pt: 8, pos: "sticky", pinY: true, overflowY: "auto", style: {
...
const TableOfContents = React.memo(({ tree, activeId, Link, maxDepthOpenByDefault, externalScrollbar = false, onLinkClick }) => {
    ...
    return (React.createElement(Box, { ref: container, w: "full", overflowY: "auto" },
...
const Item = React.memo(({ depth, isActive, id, title, meta, icon, onClick }) => {
    return (React.createElement(Flex, { id: id, bg: isActive ? 'primary-tint' : { hover: 'primary-tint' }, cursor: "pointer", pl: 4 + depth * 4, pr: 4, h: "md", align: "center", userSelect: "none", onClick: onClick, title: title },
        icon,
...
const Version = ({ value }) => {
    return (React.createElement(Box, { mr: 2 },
        React.createElement(VersionBadge, { value: value })));
};
