...
function generateProjectToC(searchResults) {
    return flow(() => searchResults, groupNodesByType, ({ articles, models, httpServices, httpOperations }) => {
        const toc = { items: [] };
        ...
        flow(() => httpServices, httpServices => appendHttpServicesToToC(toc)({
            httpServices,
            models,
            httpOperations,
...
function generateTocSkeleton(searchResults) {
    return flow(() => searchResults, groupNodesByType, ({ articles, models, httpServices }) => {
        const toc = { items: [] };
        flow(() => articles, sortArticlesByTypeAndPath, appendArticlesToToC(toc))();
        toc.items.push({ type: 'divider', title: 'APIS' });
...
function resolveHttpServices(searchResults, toc) {
    cleanToc(toc);
    flow(() => searchResults, groupNodesByType, ({ models, httpServices, httpOperations }) => {
...
function appendArticlesToToC(toc) {
    return (articles) => {
        return articles.reduce((toc, article) => flow(partial(getDirsFromUri, article.uri), findOrCreateArticleGroup(toc), group => {
...
function appendHttpServiceItemsToToC(toc) {
    return ({ httpOperations, models }, serviceTagNames) => {
        const operationsAndModels = [
            ...httpOperations.map(i => (Object.assign(Object.assign({}, i), { nodeType: NodeType.HttpOperation }))),
            ...models.map(i => (Object.assign(Object.assign({}, i), { nodeType: NodeType.Model }))),
        ];
...
export { generateProjectToC, generateTocSkeleton, isDivider, isGroup, isItem, resolveHttpServices };
