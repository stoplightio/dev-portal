...
const TryItContext = React.createContext({ hideTryIt: false });
TryItContext.displayName = 'TryItContext';
const APIWithStackedLayout = ({ serviceNode, hideTryIt }) => {
    const location = react_router_dom_1.useLocation();
    const { groups } = utils_1.computeTagGroups(serviceNode);
    return (React.createElement(TryItContext.Provider, { value: { hideTryIt } },
        React.createElement("div", { className: "w-full flex flex-col m-auto max-w-4xl" },
            React.createElement("div", { className: "w-full border-b dark:border-gray-6" },
                React.createElement(elements_core_1.Docs, { className: "mx-auto", nodeData: serviceNode.data, nodeType: types_1.NodeType.HttpService, headless: true, location: location })),
...
const Item = React.memo(({ item }) => {
    ...
    const color = elements_core_1.HttpMethodColors[item.data.method] || 'gray';
    ...
    return (React.createElement("div", { ref: scrollRef, className: classnames_1.default('w-full my-2 border border-transparent hover:border-gray-2 hover:bg-darken-1', {
        ...
        React.createElement("div", { onClick: onClick, className: "mx-auto flex items-center text-gray-7 dark:text-gray-3 hover:text-gray-8 p-2 cursor-pointer text-lg" },
            ...
            isDeprecated && React.createElement(elements_core_1.DeprecatedBadge, null)),
        React.createElement(ui_kit_1.Collapse, { isOpen: isExpanded }, hideTryIt ? (React.createElement(mosaic_1.Box, { as: elements_core_1.ParsedDocs, node: item, headless: true, p: 4 })) : (React.createElement(ui_kit_1.Tabs, { className: "PreviewTabs mx-auto", selectedTabId: tabId, onChange: (tabId) => setTabId(tabId), renderActiveTabPanelOnly: true },
            React.createElement(ui_kit_1.Tab, { id: "docs", title: "Docs", className: "p-4", panel: React.createElement(elements_core_1.ParsedDocs, { node: item, headless: true, location: location }) }),
            React.createElement(ui_kit_1.Tab, { id: "tryit", title: "Try It", className: "p-4", panel: React.createElement(elements_core_1.TryItWithRequestSamples, { httpOperation: item.data }) }))))));
});
